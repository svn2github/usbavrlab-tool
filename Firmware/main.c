 ...
 * Changes:
 *  2010-03-05 (cg)
 *      180mA Buspowerde (due to own layout with powering target)
 *		LED´s blinks even more faster during Programming ;-)
 *		created ISP_pulserst
 *      changed ISP_pulsesck to ISP_pulserst in ISP_enterprogrammingmode
 *      removed tmp=0 in value polling
 *      fixed STK500_Address problem in STK_CMD_PROGRAM_FLASH_ISP (hope so!)
 *	2009-09-23
 *		90mA Buspowered
 *		LED´s blinks faster during Programming
 *  2009-06-29
 *		STK500 Firmware saves now ISPSpeed
 *	2007-11-21
...
void OnPGMstatuschanged(uint8_t status)
{
  if (status == PGM_STATUS_PROGRAMMING)
    {
	  if (ISP_Speed != eeprom_read_byte(&ee_mem[eeispSpeed]))
