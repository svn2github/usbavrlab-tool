...
{
  unsigned char i;
  if (targetstate == TAP_STATE_UNKNOWN)
    {
	  JTAG_TAP_STATE = TAP_STATE_UNKNOWN;
	  return;
	}
...
//Data Register Scan Chain
      case TAP_STATE_SELECT_DR_SCAN:
  	    {
		  if ((targetstate >= TAP_STATE_SELECT_IR_SCAN) || (targetstate == TAP_STATE_RESET))
		    {
              JTAG_tap_state(1);
...
unsigned char JTAG_shift_bit(unsigned char TDI_on)
{
  ...
  if (JTAG_PIN & (1<<JTAG_TDI)) return 1;
  else return 0;
}
...
unsigned char JTAG_shift_bits(unsigned char data,unsigned char len)
{
  unsigned char tdo = data;
  unsigned char tdi = 0;
  JTAG_PORT &= ~(1<<JTAG_TMS);
  while(len)
   	{
      ...
      tdi >>= 1;
      if ((JTAG_PIN & (1 << JTAG_TDI)) != 0) tdi |= 0x80;
...
//  SPSR = (1 << SPI2X);
  ...
  SPCR &= ~(1 << SPE);
  chain[i] = JTAG_shift_bits(chain[i],8);
  for (i = 0; i < len / 2; i++)
    {
      unsigned char z = chain[i];
      chain[i] = chain[len - i - 1];
      chain[len - i - 1] = z;
    }
}
